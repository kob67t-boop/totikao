...
function makeBookmarklet(){
  const code = `javascript:(async()=>{try{
    ...
    for(let n=1;n<=30;n++){
      try{
        const res=await fetch(new URL('q'+n+'/',base),{credentials:'same-origin',cache:'no-store'});
        if(!res.ok)continue;
        ...
        cards.forEach((card,pos)=>{
          const textEl=card.querySelector('.seo-choice-text'); if(!textEl)return;
          const label=labels[pos]||String(pos+1);
          const ans=card.querySelector('.seo-answer');
          items.push({id:prefix+'-'+n+'-'+label,label,text:(textEl.textContent||'').replace(/\\s+/g,' ').trim(),answer:/○/.test(ans?.textContent||'')});
        });
        if(items.length)questions.push({no:n,items});
      }catch(e){}
    }
    if(!questions.length){alert('問題ページを取得できませんでした。');return}
    ...
    alert(year+'：'+questions.length+'問・'+total+'肢をコピーしました。\\n肢IDは取得順からア・イ・ウ…を自動生成しています。');
