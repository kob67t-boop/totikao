...
function makeBookmarklet(){
  const code = `javascript:(async()=>{try{
    ...
    const m=base.pathname.match(/^(.*\\/)(?:index\\.html)?$/);
    const dir=m?m[1]:base.pathname.endsWith('/')?base.pathname:base.pathname+'/' ;
    const yearMatch=dir.match(/\\/(h\\d+|r\\d+)\\/?$/i);
    if(!yearMatch){alert('年度別の問題一覧ページで実行してください。');return}
    const yearCode=yearMatch[1].toUpperCase();
    const urls=Array.from({length:20},(_,i)=>new URL('q'+(i+1)+'/',base.origin+dir).href);
    const questions=[];
    for(let n=0;n<urls.length;n++){
      try{
        const res=await fetch(urls[n],{credentials:'same-origin',cache:'no-store'});
        if(!res.ok)continue;
        const html=await res.text();
        const doc=new DOMParser().parseFromString(html,'text/html');
        let items=[...doc.querySelectorAll('.seo-choice-card')].map(card=>{
          const id=(card.querySelector('.seo-label')?.textContent||'').trim();
          const lm=id.match(/^(R|H)\\d+-(\\d+)-([ア-ン])$/);
          const text=(card.querySelector('.seo-choice-text')?.textContent||'').replace(/\\s+/g,' ').trim();
          const ans=(card.querySelector('.seo-answer')?.textContent||'').trim();
          return lm&&text?{id,label:lm[3],text,answer:/○/.test(ans)}:null;
        }).filter(Boolean);
        if(!items.length){
          const ld=[...doc.querySelectorAll('script[type="application/ld+json"]')].map(x=>{try{return JSON.parse(x.textContent)}catch(e){return null}}).find(x=>x&&x.hasPart);
          items=(ld?.hasPart||[]).map((x,i)=>({id:yearCode+'-'+(n+1)+'-'+String.fromCharCode(12450+i),label:String.fromCharCode(12450+i),text:String(x.name||''),answer:/正しい|true/i.test(String(x.acceptedAnswer?.text||''))})).filter(x=>x.text);
        }
        if(items.length){
          items.sort((a,b)=>a.id.localeCompare(b.id,'ja',{numeric:true}));
          questions.push({no:n+1,items});
        }
      }catch(e){}
    }
    questions.sort((a,b)=>a.no-b.no);
    if(!questions.length){alert('問題を取得できませんでした。元サイトの平成・令和年度の「問題一覧」ページで実行してください。');return}
    const out={year:yearCode,source:location.href,questions};
    await navigator.clipboard.writeText(JSON.stringify(out));
    const total=questions.reduce((n,q)=>n+q.items.length,0);
    const missing=[];for(let i=1;i<=20;i++)if(!questions.some(q=>q.no===i))missing.push(i);
    alert(yearCode+'：'+questions.length+'問・'+total+'肢をコピーしました。'+(missing.length?'\\n取得できなかった問：'+missing.join(','):'')+'\\n自作アプリへ貼り付けてください。');
  }catch(e){alert('取り込み中にエラー：'+e.message)}})()`;
  const box=$("bookmarklet");if(box)box.value=code;const link=$("bm");if(link)link.href=code;
}
function buildBookmarklet(){makeBookmarklet()}
