...
function buildBookmarklet(){
  const code = `javascript:(async()=>{try{
    alert('1年分取り込みを開始します。元サイトの問題ページを順番に取得します。');
    const p=location.pathname,m=p.match(/\\/(r|h)(\\d{2})(?:\\/|$)/i);
    if(!m){alert('年度ページを判定できませんでした。令和・平成の年度一覧ページで実行してください。');return}
    const prefix=m[1].toUpperCase()+m[2],year=(m[1].toLowerCase()==='r'?'令和':'平成')+parseInt(m[2],10)+'年度';
    const labels=['ア','イ','ウ','エ','オ','カ','キ','ク','ケ','コ','サ','シ','ス','セ','ソ','タ','チ','ツ','テ','ト'];
    const links=[...document.querySelectorAll('a[href]')].map(a=>{try{return [Number((new URL(a.href,location.href).pathname.match(/\\/q(\\d+)(?:\\/|\\.html?|$)/i)||[])[1]),new URL(a.href,location.href).href]}catch(e){return [0,'']}}).filter(x=>x[0]>0);
    const urls=new Map(links);
    const base=new URL('.',location.href).href;
    for(let n=1;n<=30;n++)if(!urls.has(n))urls.set(n,new URL('q'+n+'/',base).href);
    const tx=e=>(e?.textContent||'').replace(/\\s+/g,' ').trim();
    async function get(n){
      const first=urls.get(n),alts=[first,new URL('q'+n+'/',base).href,new URL('q'+n+'.html',base).href,new URL('q'+n+'/index.html',base).href];
      for(const u of [...new Set(alts)])try{const r=await fetch(u,{credentials:'same-origin',cache:'no-store'});if(r.ok){const t=await r.text();if(t.length>300)return new DOMParser().parseFromString(t,'text/html')}}catch(e){}
      return null
    }
    function parse(doc,n){
      let cards=[...doc.querySelectorAll('.seo-choice-card')];
      if(!cards.length)cards=[...doc.querySelectorAll('[class*="choice-card"]')];
      if(!cards.length)return [];
      const out=[];
      for(const card of cards){
        const textEl=card.querySelector('.seo-choice-text,[class*="choice-text"]');
        if(!textEl)continue;
        const text=tx(textEl);if(!text)continue;
        const ans=tx(card.querySelector('.seo-answer,[class*="answer"]'))||'';
        const label=labels[out.length]||String(out.length+1);
        out.push({id:prefix+'-'+n+'-'+label,label,text,answer:/○/.test(ans)});
      }
      return out
    }
    const questions=[],missing=[];
    for(let n=1;n<=30;n++){const d=await get(n);if(!d){missing.push(n);continue}const items=parse(d,n);if(items.length)questions.push({no:n,items});else missing.push(n)}
    if(!questions.length){alert('問題を1問も取得できませんでした。');return}
    const total=questions.reduce((a,q)=>a+q.items.length,0);
    await navigator.clipboard.writeText(JSON.stringify({year,source:location.href,questions}));
    alert(year+'：'+questions.length+'問・'+total+'肢をコピーしました。'+(missing.length?'\\n取得できなかった問：'+missing.join(', '):'')+'\\n肢IDは取得順から自動生成しています。');
  }catch(e){alert('取り込み中にエラー：'+(e&&e.message?e.message:e))}})()`;
  const link=$("bm"), box=$("bookmarklet");
  if(link)link.href=code;
  if(box)box.value=code;
}

function boot(){buildBookmarklet();try{qs=JSON.parse(localStorage.getItem(QK)||"null")||JSON.parse(JSON.stringify(INITIAL_QUESTIONS));}catch(e){qs=JSON.parse(JSON.stringify(INITIAL_QUESTIONS));}pg=JSON.parse(localStorage.getItem(PK)||"{}");document.querySelectorAll(".tabs button").forEach(b=>b.onclick=()=>show(b.dataset.v));$("yes").onclick=()=>ans(true);$("no").onclick=()=>ans(false);$("nx").onclick=()=>{idx++;drawQuiz()};$("parse").onclick=parseSource;$('paste').onclick=async()=>{try{$("src").value=await navigator.clipboard.readText()}catch(e){alert("クリップボードを読み取れませんでした")}};if($("jsonImport"))$("jsonImport").onclick=importJSON;$('copyCode').onclick=()=>navigator.clipboard.writeText($('bookmarklet').value).then(()=>alert("コピーしました"));$('clearSrc').onclick=()=>$("src").value="";$('add').onclick=()=>editQ();$('save').onclick=saveQ;$('cancel').onclick=()=>$("editor").classList.add("hidden");$('del').onclick=delQ;$('reset').onclick=reset;buildBookmarklet();show("home")}
