...
function makeBookmarklet(){
  const code = `javascript:(async()=>{try{
    const links=[...document.querySelectorAll('a[href]')]
      ...
      .filter(x=>/^問\\s*\\d+/.test(x.t));
    const urls=[...new Map(links.map(x=>[x.u,x])).values()];
    if(!urls.length){alert('問1〜のリンクが見つかりません。年度一覧ページを開いてください。');return}
    const year=(document.title.match(/令和\\d+年度|平成\\d+年度/)||['年度不明'])[0];
    const questions=[];
    for(const x of urls){
      try{
        const html=await (await fetch(x.u,{credentials:'same-origin'})).text();
        const doc=new DOMParser().parseFromString(html,'text/html');
        const items=[];
        for(const e of doc.querySelectorAll('[id]')){
          const m=(e.id||'').match(/^(R|H)\\d+-(\\d+)-([ア-ン])$/);
          if(!m) continue;
          const root=e.closest('article,section,main')||e.parentElement||doc.body;
          const lines=(root.innerText||'').split(/\\n/).map(v=>v.trim()).filter(Boolean);
          const k=lines.findIndex(v=>v===e.id);
          let z=k>=0?lines.slice(k+1):lines;
          const ai=z.findIndex(v=>/^解答\\s*[:：]?\\s*[○×]/.test(v));
          const answer=ai>=0?/○/.test(z[ai]):null;
          z=z.slice(0,ai>=0?ai:z.length)
            .filter(v=>!/^(R|H)\\d+-\\d+-[ア-ン]$/.test(v))
            .filter(v=>!/^[ア-ン]$/.test(v))
            .filter(v=>!/^出典\\s*[:：]/.test(v));
          const text=z.join(' ').replace(/\\s+/g,' ').trim();
          if(text) items.push({id:e.id,label:m[3],text,answer});
        }
        items.sort((a,b)=>a.id.localeCompare(b.id,'ja',{numeric:true}));
        if(items.length){
          const no=Number((items[0].id.match(/^[RH]\\d+-(\\d+)-/)||[])[1]);
          questions.push({no,items});
        }
      }catch(e){}
    }
    if(!questions.length){alert('問題ページを取得できませんでした。元サイトの年度一覧ページで実行してください。');return}
  ...
  const box=$("bookmarklet"); if(box) box.value=code;
...
window.imp=n=>{const q=window._found[n],i=qs.findIndex(x=>x.year===q.year&&x.no===q.no);if(i>=0)qs[i]=q;else qs.push(q);save();$("importResult").innerHTML=`<p class=ok>${q.year}・問${q.no}を登録しました。</p>`}
window.impAll=()=>{for(const q of(window._found||[])){const i=qs.findIndex(x=>x.year===q.year&&x.no===q.no);if(i>=0)qs[i]=q;else qs.push(q)}save();$("importResult").innerHTML=`<p class=ok>${window._found.length}問をまとめて登録しました。</p>`}
