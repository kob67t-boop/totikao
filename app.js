...
function buildBookmarklet(){
  const code = `javascript:(async()=>{try{
    alert('1年分取り込みを開始します。');
    const here=location.href, title=document.title||'', body=(document.body?.innerText||'');
    const links=[...document.querySelectorAll('a[href]')];
    const all=[here,title,body,...links.map(a=>a.href+' '+(a.innerText||''))].join('\\n');
    let m=all.match(/(?:^|[^a-z])([rh])\\s*0*(\\d{1,2})(?:年度)?/i);
    if(!m){
      const ym=prompt('年度を判定できませんでした。平成18なら H18、令和7なら R7 と入力してください。','');
      if(!ym){alert('取り込みを中止しました。');return}
      m=ym.trim().match(/^(r|h)\\s*0*(\\d{1,2})$/i);
    }
    if(!m){alert('年度を判定できませんでした。H18 または R7 の形式で指定してください。');return}
    const prefix=m[1].toUpperCase()+String(parseInt(m[2],10)).padStart(2,'0');
    ...
    for(const a of links){
      try{
        ...
        const mm=new URL(u).pathname.match(/(?:^|\\/)q(\\d+)(?:\\/|\\.html?|$)/i);
        if(mm)linkMap.set(Number(mm[1]),u);
      }catch(e){}
    }
    const base=new URL('.',location.href).href;
    async function getDoc(n){
      const candidates=[];
      if(linkMap.has(n))candidates.push(linkMap.get(n));
      candidates.push(new URL('q'+n+'/',base).href,new URL('q'+n+'.html',base).href,new URL('q'+n+'/index.html',base).href);
      for(const u of [...new Set(candidates)]){
        try{
          const r=await fetch(u,{credentials:'same-origin',cache:'no-store'});
          if(r.ok){
            const t=await r.text();
            if(t.length>200)return new DOMParser().parseFromString(t,'text/html');
          }
        }catch(e){}
      }
      return null;
    }
    function clean(e){return(e?.textContent||'').replace(/\\s+/g,' ').trim()}
    function parse(doc,n){
      ...
      for(const card of cards){
        const te=card.querySelector('.seo-choice-text,[class*="choice-text"]');
        if(!te)continue;
        const text=clean(te);if(!text)continue;
        const ae=card.querySelector('.seo-answer,[class*="answer"]');
        const label=labels[out.length]||String(out.length+1);
        out.push({id:prefix+'-'+n+'-'+label,label,text,answer:/○/.test(clean(ae))});
      }
      return out;
    }
    const questions=[],missing=[];
    for(let n=1;n<=30;n++){
      const d=await getDoc(n);
      if(!d){missing.push(n);continue}
      const items=parse(d,n);
      if(items.length)questions.push({no:n,items});else missing.push(n);
    }
    if(!questions.length){alert('問題を1問も取得できませんでした。年度一覧ページを開いて実行してください。');return}
  ...
  }catch(e){alert('取り込み中にエラー：'+(e?.message||String(e)))}})()`;
